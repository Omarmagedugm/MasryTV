...
export function useFirestoreSync() {
  ...
  // Use uid from store (set by App.tsx onAuthStateChanged) for reliable reactivity
  const currentUid = useAppStore(state => state.profile?.uid);
  ...
  }, [currentUid]); // currentUid comes from Zustand store (set by onAuthStateChanged in App.tsx)
}
